...
async function processSelfCarePoints(targetDate) {
  try {
    ...
    // Write points to Google Sheets - each task has its own row
    try {
      ...
      // Each task maps to a row and which emoji(s) contribute to it
      const taskRows = [
        { row: 23, name: 'Check-In',  emojis: [CHECKIN_EMOJI] },
        { row: 24, name: 'Teeth',     emojis: ['🪥', '🦷'] },
        { row: 25, name: 'Bed',       emojis: ['🛏️'] },
        { row: 26, name: 'Hair',      emojis: ['👑'] },
        { row: 27, name: 'Meds',      emojis: ['💊', '⚕️'] },
        { row: 28, name: 'Outfit',    emojis: ['👕'] },
        { row: 29, name: 'Wash',      emojis: ['🚿'] },
        { row: 30, name: 'Water',     emojis: ['🥛'] },
        { row: 31, name: 'Food',      emojis: ['🍕'] },
        { row: 32, name: 'Read',      emojis: ['📖'] },
      ];
        ...
        try {
          for (const task of taskRows) {
            ...
            await sheets.spreadsheets.values.update({
              ...
              requestBody: { values: [[current + taskPoints]] }
            });
          }
          console.log(`Successfully wrote ${house.name} self-care points to Sheets.`);
        } catch (err) {
          console.error(`Failed to write ${house.name} self-care points to Sheets:`, err);
        }
      }
    } catch (err) {
      console.error('Failed to connect to Sheets for self-care points:', err);
    }
...
console.log('Common room message IDs restored!');
  ...
  // 7AM ET = 12:00 UTC - collects previous day's self-care points
  // Skips on the 1st of the month (last day of previous month handled by the 11PM cron)
